...
int dcmdump(int argc, char *argv[], const char **output)
{
    ...
    while (if_iter != if_last)
    {
      ...
      if (printFilename)
      {
        ...
        buffer << "# " << OFFIS_CONSOLE_APPLICATION << " (" << fileCounter << "/" << count << "): " << current << OFendl;
      }
      errorCount += dumpFile(buffer, current, readMode, xfer, printFlags, loadIntoMemory, stopOnErrors, convertToUTF8, stopParsingBeforeElement, pixelDirectory);
    }

    std::string bufferContent = buffer.str();
    const char* charArray = bufferContent.c_str();
    *output = charArray;
